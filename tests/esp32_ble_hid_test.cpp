...
// The HID adapter must create its services on the shared server before the
...
void test_hid_begin_advertises_keyboard() {
    ...
    assert(hid.begin("AirVIA WROOM32", "AirVIA"));
    ...
    assert(Nimble.advertisingObj.name() == "AirVIA WROOM32");
}
